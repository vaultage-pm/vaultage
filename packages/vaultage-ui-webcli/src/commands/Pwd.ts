...
export class PwdCommand implements ICommand {
    ...
    public async handle() {
        ...
        try {
            let newMasterPassword = await this.shell.promptSecret('New master password          :');
            let newMasterPassword2 = await this.shell.promptSecret('New master password (confirm):');
            ...
            await new Promise((resolve, reject) => this.vault.updateMasterPassword(newMasterPassword, function(err) {
                if(err == null){
                    resolve()
                } else {
                    reject(err)
